...
from SourceCode.simulation.nn.preprocessing import data_preparation, split_feedforward_sequence
...
inputs = 2
input_dim = inputs*seq_length  # time series-input dimension
output_dim = 1

hidden_dim = 24
layer_dim = None  # No. layers is set in the class
inx = inputs - output_dim

model = FFNN(input_dim, hidden_dim, output_dim)  # instantiate model class
...
# Train data
X, y = split_feedforward_sequence(train_data, seq_length)
...
for epoch in range(num_epochs):
    for in_x, out_y in train_loader:
        inputs_xb = torch.clone(in_x.view((-1, input_dim)))
    ...
    for xb_test, yb_test in test_loader:
        xb_intest = torch.clone(xb_test.view((-1, input_dim)))
...
# PREDICT MODEl
predicted = (model(X2.view(-1, input_dim))).data.numpy()
